...
- Access remotely via Tailscale: the container's Caddy binds all interfaces, so a host running `make dev` is reachable at `http://<your-tailnet-hostname>:28800` (no `API_HOST` override needed)
- **DB**: Postgres 18, containerized alongside the API/web/Caddy in `deploy/docker-compose.local.yml` for `make dev`; a separate bare Postgres via `db/docker-compose.yml` backs `make test-db`/`make ci` only
