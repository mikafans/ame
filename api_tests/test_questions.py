...
def test_list_versions(client, auth, question_id):
    resp = client.get(f"/questions/{question_id}/versions", headers=auth)
    assert resp.status_code == 200
    assert isinstance(resp.json()["versions"], list)


def test_promote_question(client, auth, question_id):
    resp = client.post(f"/questions/{question_id}/promote", headers=auth)
    assert resp.status_code == 200
    assert resp.json()["status"] == "live"


def test_archive_question(client, auth, question_id):
    resp = client.post(f"/questions/{question_id}/archive", headers=auth)
    assert resp.status_code == 200
    assert resp.json()["status"] == "archived"


def test_get_nonexistent_question(client, auth):
    resp = client.get("/questions/00000000-0000-0000-0000-000000000000", headers=auth)
