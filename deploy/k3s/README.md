...
Build infrastructure for the container images the k3s deployment runs. Builds
default to `linux/amd64` (x64); set `PLATFORM=linux/arm64` for the harus-pi
edge. The
...
amd64 and only the COPY-only runtime stage is the target arch, so **no
qemu/binfmt** is needed. Run from `harus-mini` (amd64), with `podman login
docker.io` as `azusachino`.
