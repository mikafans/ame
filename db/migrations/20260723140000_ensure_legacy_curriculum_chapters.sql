-- M5 correction: baseline-era journeys may have activities but no chapters.

-- Create the missing Foundations chapter before grouping the legacy activities.
INSERT INTO tb_journey_chapters (
    journey_id,
    subject_user_id,
    title,
    summary,
    order_index
)
SELECT
    j.id AS journey_id,
    j.subject_user_id,
    'Foundations' AS title,
    format(
        'Build the essential model and vocabulary for %s.',
        g.normalized_statement
    ) AS summary,
    0 AS order_index
FROM tb_learning_journeys AS j
INNER JOIN tb_learning_goals AS g ON j.goal_id = g.id
WHERE
    NOT EXISTS (
        SELECT 1
        FROM tb_journey_chapters AS existing_chapter
        WHERE existing_chapter.journey_id = j.id
    )
    AND EXISTS (
        SELECT 1
        FROM tb_activities AS legacy_activity
        WHERE legacy_activity.journey_id = j.id
    )
ON CONFLICT (journey_id, order_index) DO NOTHING;

-- Attach the first two ungrouped starter activities to Foundations.
UPDATE tb_activities AS activity
SET chapter_id = foundations.id
FROM tb_journey_chapters AS foundations
WHERE
    foundations.journey_id = activity.journey_id
    AND foundations.order_index = 0
    AND activity.chapter_id IS NULL
    AND activity.order_index < 2;

-- Create Practice and application for the remaining ungrouped activities.
INSERT INTO tb_journey_chapters (
    journey_id,
    subject_user_id,
    title,
    summary,
    order_index
)
SELECT
    j.id AS journey_id,
    j.subject_user_id,
    'Practice and application' AS title,
    format(
        'Use the model in examples and a bounded task for %s.',
        g.normalized_statement
    ) AS summary,
    1 AS order_index
FROM tb_learning_journeys AS j
INNER JOIN tb_learning_goals AS g ON j.goal_id = g.id
INNER JOIN tb_journey_chapters AS foundations
    ON
        j.id = foundations.journey_id
        AND foundations.order_index = 0
WHERE
    EXISTS (
        SELECT 1
        FROM tb_activities AS later_activity
        WHERE
            later_activity.journey_id = j.id
            AND later_activity.order_index >= 2
            AND later_activity.chapter_id IS NULL
    )
    AND NOT EXISTS (
        SELECT 1
        FROM tb_journey_chapters AS existing_chapter
        WHERE
            existing_chapter.journey_id = j.id
            AND existing_chapter.order_index = 1
    )
ON CONFLICT (journey_id, order_index) DO NOTHING;

-- Preserve authored multi-chapter paths while grouping remaining baseline
-- activities into the newly-created practice chapter.
WITH legacy_journeys AS (
    SELECT
        foundations.journey_id,
        foundations.id AS foundation_id,
        practice.id AS practice_id
    FROM tb_journey_chapters AS foundations
    INNER JOIN tb_journey_chapters AS practice
        ON
            foundations.journey_id = practice.journey_id
            AND practice.order_index = 1
    WHERE
        foundations.order_index = 0
        AND NOT EXISTS (
            SELECT 1
            FROM tb_activities AS authored_activity
            WHERE
                authored_activity.journey_id = foundations.journey_id
                AND authored_activity.chapter_id IS NOT NULL
                AND authored_activity.chapter_id <> foundations.id
        )
)

UPDATE tb_activities AS activity
SET chapter_id = legacy.practice_id
FROM legacy_journeys AS legacy
WHERE
    activity.journey_id = legacy.journey_id
    AND activity.chapter_id = legacy.foundation_id
    AND activity.order_index >= 2;

WITH legacy_journeys AS (
    SELECT
        foundations.journey_id,
        foundations.id AS foundation_id,
        practice.id AS practice_id
    FROM tb_journey_chapters AS foundations
    INNER JOIN tb_journey_chapters AS practice
        ON
            foundations.journey_id = practice.journey_id
            AND practice.order_index = 1
    WHERE
        foundations.order_index = 0
        AND NOT EXISTS (
            SELECT 1
            FROM tb_activities AS authored_activity
            WHERE
                authored_activity.journey_id = foundations.journey_id
                AND authored_activity.chapter_id IS NOT NULL
                AND authored_activity.chapter_id <> foundations.id
        )
)

UPDATE tb_activities AS activity
SET chapter_id = legacy.practice_id
FROM legacy_journeys AS legacy
WHERE
    activity.journey_id = legacy.journey_id
    AND activity.chapter_id IS NULL
    AND activity.order_index >= 2;
