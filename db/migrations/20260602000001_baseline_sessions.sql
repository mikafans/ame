...
CREATE INDEX idx_sessions_assessment_finished ON tb_sessions (assessment_id, finished_at DESC) WHERE status
= 'finished';
...
CREATE INDEX tb_activity_log_grade_count ON tb_activity_log (agent_id) WHERE tool_name = 'attempt.grade'
AND status = 200;
